...
describe("NAV submission default (adatszolgáltatás is mandatory, so it follows the company setup)", () => {
  const NAV_READY = {
    id: "c1",
    userId: "u1",
    name: "Demo Kft.",
  ...
  it("defaults the NAV toggle on when the company has NAV credentials on file", async () => {
    setupApiFetch({ company: NAV_READY });
    const ref = await renderComposer({ mode: "create" });

    expect(ref.current!.navEnabled).toBe(true);
  });

  it("leaves it off when NAV is not configured yet", async () => {
    setupApiFetch({ company: { ...NAV_READY, navTechnicalUser: undefined, navTechnicalPasswordSet: false } });
    const ref = await renderComposer({ mode: "create" });

    expect(ref.current!.navEnabled).toBe(false);
  });

  it("does not force the toggle back on when the user turns it off", async () => {
    setupApiFetch({ company: NAV_READY });
    const ref = await renderComposer({ mode: "create" });

    await act(async () => {
      ref.current!.setNavEnabled(false);
    });
