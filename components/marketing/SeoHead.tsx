...
function JsonLdScripts({ graphs }: { graphs: object[] }) {
  if (graphs.length === 0 || Platform.OS !== "web") {
    return null;
  }
  ...
  return (
    <>
      {graphs.map((graph, index) => (
        <script
          // Body JSON-LD is valid for crawlers and reliable under Expo web.
          // eslint-disable-next-line react/no-danger
          key={`jsonld-body-${index}`}
