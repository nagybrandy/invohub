...
const plan = await agent(
  RULES +
    ...
    'Keep the plan small enough for one Sonnet implementer to finish in one run. Then in docs/loop-queue.md mark the chosen item "[~] folyamatban (slice/<slug>)" and commit both files ("Plan: <slug>"). ' +
    'IMPORTANT — you are working in the shared MAIN checkout, not an isolated worktree (concurrent human/agent work may be happening there at the same time): do NOT run `git checkout -b` or switch branches at all. Commit the plan doc and the loop-queue.md update directly on whatever branch is currently checked out (same as the Research phase already does) — the actual `slice/<slug>` branch is created later, by the Build phase, in its own isolated worktree. Report the intended branch name as the `branch` field in your structured output; do not create it yourself.',
