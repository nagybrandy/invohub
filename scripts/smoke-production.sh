...
for attempt in $(seq 1 "$MAX_ATTEMPTS"); do
  ...
  entry_path="$(grep -oE '/_expo/static/js/web/entry-[^"]+\.js' /tmp/invohub-home.html | head -1 || true)"
  entry_code="000"
  if [ -n "$entry_path" ]; then
    entry_code="$(curl -s -o /dev/null -w "%{http_code}" "${BASE_URL}${entry_path}" || true)"
  fi

  if [ "$health_code" = "200" ] && [ "$home_code" = "200" ] && [ "$entry_code" = "200" ]; then
    echo "Health:"
  ...
  echo "  health=${health_code} home=${home_code} entry=${entry_code} — retrying in ${SLEEP_SECONDS}s"
  sleep "$SLEEP_SECONDS"
done
